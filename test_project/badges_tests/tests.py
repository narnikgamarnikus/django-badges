...
from badges.templatetags.badges_tags import (badge_count, level_title,
                                             level_count)
from badges.receivers import sync_badges_to_db
...
class BadgeTests(TestCase):
    ...
    def test_badge_registration_only_happens_once(self):
        ...
        self.assertTrue(
            registered_badges[meta_badge.id] is meta_badge_instance)
    ...
    def test_badge_progress(self):
        ...
        self.assertEqual(
            badge.meta_badge.get_progress_percentage(user=user), 0.0)
        self.assertRaises(RequiresUserOrProgress,
                          badge.meta_badge.get_progress_percentage)
        ...
        self.assertEqual(
            badge.meta_badge.get_progress_percentage(user=user), 100.0)
    ...
    def test_badge_earned_signal(self):
        ...
        self.assertTrue(isinstance(signal_handler_kwargs.get(
            'sender'), self.meta_badge.__class__))
        self.assertEqual(signal_handler_kwargs.get('user'), user)
        self.assertEqual(signal_handler_kwargs.get(
    ...
    def test_template_tags(self):
        ...
        self.assertEqual(badge_count(user), [
            {
                'count': 0,
                'badge__level': '1'
            },
