...
def badge_count(user_or_qs=None):
    ...
    def get_badge_count(level):
        ...
        if bc:
            ...
            # if the user has no badges at this level,
...
class MetaBadge(with_metaclass(MetaBadgeMeta, object)):
    ...
    def _test_conditions(self, instance):
        condition_callbacks = [getattr(self, c)
                               for c in dir(self) if c.startswith('check')]
    ...
    def get_progress_percentage(self, progress=None, user=None):
        if user is None and progress is None:
            raise RequiresUserOrProgress(
                "This method requires either a user" +
                " or progress keyword argument")
        ...
        current_progress = (self.progress_finish - self.progress_start)
        return (100.0 * progress) / current_progress
